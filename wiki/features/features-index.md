...
| **Backup & restore** | [`feat-backup.md`](feat-backup.md) | desktop | A folder copied somewhere safe on a schedule: copy-only with versions kept, a per-device destination so two machines cannot merge, retention with a prune that refuses rather than guesses, and a restore that is just the file browser plus the existing conflict preflight. |

...
| **Tree view** | [File Browser §6.2](feat-file-browser.md#62-the-tree-view) · [Explorer design § View Modes](../core/20-explorer-design.md#-view-modes) · [plan](../../dev/plans/tree-view-plan.md) | desktop | shipped | The fourth `ViewMode`: an expandable hierarchy in one pane — lazy per-folder listings, several folders open at once, Details columns, keyboard expand/collapse, selection spanning folders, and every operation resolving its path from the **node**, never `state.path`. Not offered on the touch shell. |

...
| **Add a cloud (guided setup)** | [`feat-add-a-cloud.md`](feat-add-a-cloud.md) | all | shipped | The guided default for adding storage: ten curated tiles, hand-written essentials for the backends rclone asks nothing about, and OAuth as a real screen — one primary button, every other way that can actually succeed behind it, cancel that frees port 53682 **and** removes the half-written remote. Today's full option form is kept as **Advanced**, reachable from three places. Both are one driver over rclone's own interactive config machine. |
...
| **Media playback** | [`feat-media-playback.md`](feat-media-playback.md) | all | shipped | Audio and video played straight out of a remote through the engine's authenticated URL (libmpv, so it handles what a browser will not). Owns the three player surfaces — the video frame, the pointer audio card, and the **television** D-pad overlay and now-playing screen — plus the transport-key bindings honoured on every platform, previous/next walking its own media kind, repeat, and the one error card every failure path lands on. |

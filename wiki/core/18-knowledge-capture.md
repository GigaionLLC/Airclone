...
| Store-facing release copy (the line users see in a store listing) | [`docs/store/store-release-notes.txt`](../../docs/store/store-release-notes.txt) | One generic line under 500 bytes, reused every release. Apple's `whatsNew` is per-version and starts empty each time, so [`tool/asc_listing.py`](../../tool/asc_listing.py) sends this file as part of a listing push (`--apply`; without it the run only reports); Microsoft's field is **cloned** from the previous submission and is the one edited by hand, in Partner Center. [`docs/store/README.md`](../../docs/store/README.md) owns the per-store detail. |

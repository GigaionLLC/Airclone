...
| `parseAuthUrl` / `asAuthUrl` | [`oauth_flow.dart`](../../packages/airclone_rc/lib/src/oauth_flow.dart) | Whether a string is a URL this app may hand to the platform browser. Scheme, host, port and path are all pinned to rclone's own constants and a `state` is required, so a log line can yield rclone's loopback listener or nothing at all — never an address of someone else's choosing. Engine output is not a trusted channel: at high verbosity rclone echoes back remote names and server error text. |
| `matchesProvider` / `optionsFor` | [`models/provider.dart`](../../packages/airclone_rc/lib/src/models/provider.dart) | Which of a backend's options apply once `provider` is chosen — rclone's `fs.MatchProvider`, ported exactly rather than approximated, because guessing wrong hides a field somebody needs. Verified against rclone's own test table. |
| `recipeFor` / `cloudMatches` / `suggestRemoteName` | [`remote_setup_recipes.dart`](../../app/lib/src/state/remote_setup_recipes.dart) | The curated layer: which fields a backend rclone asks nothing about actually needs, which search words find which cloud, and a unique remote name nobody had to invent. Pure data, checked against a real `config/providers` capture so an invented option name cannot ship. |

...
void main() {
  test(
    'retry replays the exact RC call as a new job, original untouched',
    () async {
      ...
      c
          ...
          .markDone(original.id, JobStatus.failed);
      expect(c.read(jobsControllerProvider).first.canRetry, isTrue);

      final copyCallsBefore = client.calls
          .where((x) => x.method == 'operations/copyfile')
          .length;
      ...
      expect(
        after.firstWhere((j) => j.id == original.id).status,
        JobStatus.failed,
      );

      // The retry re-dispatched the SAME copyfile call (one more than before).
      final copyCalls = client.calls
          .where((x) => x.method == 'operations/copyfile')
          .toList();
  ...
  test(
    'retry is a no-op for a job that never dispatched (no rcMethod)',
    () async {
      final c = ProviderContainer();
      addTearDown(c.dispose);

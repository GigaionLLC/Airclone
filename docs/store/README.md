...
| **Mac App Store** | **IN PROGRESS**, never submitted. Account, pricing ($1.49) and listing metadata are done; a sandboxed build is verified RUNNING on CI hardware. Build+upload lane written but unproven. | [`apple/listing-en-US.md`](apple/listing-en-US.md) (draft) | not captured yet | [`dev/plans/apple-appstore-plan.md`](../../dev/plans/apple-appstore-plan.md) |
| **iOS App Store** | **BLOCKED** on an iOS librclone build. Off-road upstream, but proven by third parties — see the plan's Gate C2. Do not start a submission lane. | — | — | [`dev/apple-appstore-and-macos.md`](../../dev/apple-appstore-and-macos.md) §2 |

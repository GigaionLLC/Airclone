...
| **Submit to Google Play** | [`google-play-store.md`](google-play-store.md) | Per-release runbook. Open testing is [`publish-play.yml`](../.github/workflows/publish-play.yml) and production is [`promote-play.yml`](../.github/workflows/promote-play.yml) — both buttons, neither a tag side-effect (open testing was automatic on every tag until v0.8.3). One-time service-account setup is [`play-ci-setup.md`](play-ci-setup.md) — not the 2026-07 research in `plans/store-automation-plan.md`, which predates the live lane. |
...
| **Turn on in-app updating** | [`update/signing-key-setup.md`](update/signing-key-setup.md) | Not done yet, and deliberately a person's job: the release signing key is created on a machine you control and never touches this repo. Until it exists, releases publish an UNSIGNED `SHA256SUMS` and the app offers its release page instead of installing anything. The design behind it is [`plans/self-update-plan.md`](plans/self-update-plan.md). |
...
| **Reply to a bug report** | [`bug-reports.md`](bug-reports.md) | Plain and short: it's fixed, what went wrong in their terms, what to download. The mechanism belongs in the commit, not the reply. Close the issue with the reply. |
...
| [`bug-reports.md`](bug-reports.md) | How to write the reply on a GitHub issue: what the four parts of it are, what to leave out, and the worked example from issue #3. Short doc, read it before answering a reporter. |
...
rare irreversible act), and **monitoring** (a scheduled read of what a store, or upstream, is doing to us). Most of
...
| :--- | :--- | :--- |
| [`ci.yml`](../.github/workflows/ci.yml) | push to `main`, every PR, manual, weekly cron (Mondays 06:00 UTC) | Four jobs. `analyze-test`: `dart format --set-exit-if-changed` → `flutter analyze` → `flutter test --coverage` (coverage uploaded as an artifact). `docs` (no Flutter, seconds): [`tool/check-docs.py`](../tool/check-docs.py) — **hard fail** on a broken relative link (including links into `app/` source) or a control byte in a doc, orphans and doc shape advisory without `--strict` — then `python -m compileall tool/`, then [`tool/check-workflows.py`](../tool/check-workflows.py) — **hard fail** on an empty GitHub expression (which invalidates a whole workflow file, comments included) or a free-form dispatch input interpolated straight into a `run:` block. `package-airclone-rc`: analyze and test the in-repo [`packages/airclone_rc`](../packages/airclone_rc/) package against the same pinned Flutter SDK, so the app and the package are judged by one toolchain — on **ubuntu, windows and macos**, because the package is where the OS shows through (the reaper kills by PID, the object server binds loopback, the spawned engine is a child with pipes), and on each one it downloads the pinned rclone, verifies its checksum and runs the live spawned-engine test against it. Format, analyze and the never-imports-the-app guard run on the Linux leg only, being platform-independent. `rclone-pin`: **warn-only** drift check comparing `RCLONE_VERSION` in `release.yml` against the pins in `dev/android/build-rclone.ps1` and both `dev/desktop/build-librclone.*`, and against the latest upstream rclone. |
| [`release.yml`](../.github/workflows/release.yml) | push of a `v*` tag; manual runs build artifacts only (no Release) | `release` (creates the GitHub Release first, so platform jobs only upload) → `librclone` matrix → `windows`, `linux`, `macos`, `android` in parallel. `RCLONE_VERSION` is pinned once at workflow level for every engine build. It publishes to **no store**: since v0.8.3 the `android` job builds and attaches the AAB, and sending it to Play open testing is the separate [`publish-play.yml`](../.github/workflows/publish-play.yml). |
...
| [`publish-airclone-rc.yml`](../.github/workflows/publish-airclone-rc.yml) | push of an `airclone_rc-v*` tag; manual runs validate only | Publishes [`packages/airclone_rc`](../packages/airclone_rc) to pub.dev over OIDC — no API key here or in secrets. Re-runs format, analyze, tests and a **zero-warning** `dart doc` build, checks the tag against the pubspec version, and refuses while `publish_to: none` is still in the pubspec (the guard against an accidental publish). A manual dispatch stops after the dry run, so it is safe to exercise. **Publishing is permanent** — a version can be retracted, never replaced. Two one-time setup steps belong to a person: see [the plan's Milestone C](plans/airclone-rc-package-plan.md). |
...
| :--- | :--- | :--- |
| [`publish-play.yml`](../.github/workflows/publish-play.yml) | — (`r0adkll/upload-google-play`) | Downloads the exact `airclone-playstore.aab` already attached to a release tag — it never rebuilds — and uploads it to Play **open testing**, then asks Play whether that version code really landed (`tool/play_tracks.py --expect`). `mode: dry-run` \| `upload`. Was automatic on every tag until v0.8.3; see [What a tag does](#-what-a-tag-does-by-itself-and-what-needs-a-button). |
| [`submit-msstore.yml`](../.github/workflows/submit-msstore.yml) | [`tool/store_submit.py`](../tool/store_submit.py) | Downloads the exact `airclone.msix` already attached to a release tag — it never rebuilds — and puts it in front of Microsoft. `mode: dry-run` \| **`stage`** \| `submit`; see the mode table below, and use `stage`. |
...
| [`asc-submit-review.yml`](../.github/workflows/asc-submit-review.yml) | `asc_build.py` + `asc_listing.py` | The point of no return — optionally re-pushes the listing, audits, then **Add for Review**. `confirm_version` must be typed exactly, and it pins both the listing refresh and the submit to that same version. It does **not** release: `releaseType` stays MANUAL, so an approved version waits for a human. Export compliance is not asked — it is declared by `ITSAppUsesNonExemptEncryption` in both `Info.plist` files. |
| [`asc-release.yml`](../.github/workflows/asc-release.yml) | [`tool/asc_build.py`](../tool/asc_build.py) `--release` | **Publishes an APPROVED version.** The last step of the Apple lane and the only one that makes the app public. Refuses every state except `PENDING_DEVELOPER_RELEASE`, so it cannot submit, attach, edit metadata or rescue a rejection. `mode: dry-run` \| `release`, and `release` needs the version string typed exactly. No undo — pulling a release back means removing the version from sale. |
...
| Workflow | Trigger | What it does |
| :--- | :--- | :--- |
| [`cla.yml`](../.github/workflows/cla.yml) | every pull request (`pull_request_target`), and a comment carrying the signing phrase or `recheck` | The merge gate for outside contributions ([`tool/cla_check.py`](../tool/cla_check.py)). Requires **every commit author**, not just the opener, to have signed [`CLA.md`](../CLA.md); records signatures on the orphan `cla-signatures` branch with a permalink to the exact text signed; exempts maintainers by numeric account id and bots by type. The trigger hands it a write token, so it **never checks out or runs pull request code** — it sparse-checks-out `tool/` from the trusted base. That same trigger means an edit to this file is only ever exercised once it is on `main`: its own pull request is checked by the old copy. |
| [`linux-runner.yml`](../.github/workflows/linux-runner.yml) | manual, or a push/PR touching `app/linux/**`, `dev/linux/**` or `app/pubspec.yaml` | Four jobs, because `ci.yml` never compiles `app/linux/runner/*.cc`: `compile` — which is what would have caught v0.13.4 shipping a call to the private `fl_engine_start` — plus `mount-probe`, `popout` (multi-window behaviour under Xvfb) and `flatpak` (opt-in, slow). **`popout` carries `continue-on-error: true`, so the run badge stays green while it fails.** Read the job result, not the badge. |
| [`macos-runner.yml`](../.github/workflows/macos-runner.yml) | manual, or a push/PR touching `app/macos/**`, `app/lib/main.dart`, the headless/webui sources or `dev/macos/**` | Builds the direct-download macOS app on a real Mac and proves the unattended flags put **nothing on screen**: the window comes from the nib, so AppKit shows it whatever the Dart entrypoint decides. Nobody on this project owns a Mac, so no other surface can answer. The script fails rather than passes when a plain launch shows no window either, so a runner that cannot see windows cannot manufacture a green. |
| [`windows-runner.yml`](../.github/workflows/windows-runner.yml) | manual, or a push/PR touching `app/lib/**`, `app/integration_test/**`, `app/pubspec.yaml` or `packages/airclone_rc/**` | Runs the **real app on a real Windows desktop against a real rclone** — engine start, typed rc calls, app state, rendered widgets. Every other test of the app answers `rpc` with a fake, which is what makes the unit suite run in a minute and also what it cannot tell you. Once the engine layer moved out into `packages/airclone_rc`, nothing else in CI asked whether the app, the package and a real rclone still fit together. |
| [`linux-screenshot.yml`](../.github/workflows/linux-screenshot.yml) | manual only (`seconds` input for settle time) | Runs the Linux build headlessly under Xvfb and uploads a **PNG of the window**, so a Linux visual bug can be looked at instead of reasoned about from a screenshot a user sent. Deliberately not a step in `release.yml`: driving a GUI under Xvfb is the flakiest thing in this repo's CI, and a flaky step must never be able to turn a release red. |
...
| Workflow | Trigger | What it does |
| :--- | :--- | :--- |
| [`rclone-bump.yml`](../.github/workflows/rclone-bump.yml) | weekly cron (Mondays 07:00 UTC) + manual | Opens a **pull request** when upstream rclone moves ahead of the pin, rather than pushing: resolving `latest` at release time would ship an engine nobody had run, and two builds of one tag could differ. The version is pinned in four places, and `ci.yml`'s `rclone-pin` job only warns — a warning in a CI log is precisely how this repo shipped three releases with no bundled rclone. It deliberately leaves the prose comment above the pin alone: that records *why* a version was chosen, and a person has to redo that reasoning. |
...
| :--- | :--- | :--- |
| **Play → open testing** | Actions → *Publish to Google Play (open testing)*, `mode: upload` | open testing is **public**, so a tag pushed for artifacts or an `-rc` would otherwise reach real testers — and Play burns a version code permanently on upload |
...
| **Apple: release an approved version** | Actions → *Release an approved version (Apple)*, `mode: release` | `releaseType` is MANUAL on purpose: approval and publication being the same event is how a version ships before anyone looks at it. Still a button someone presses — it just no longer needs a Console visit |

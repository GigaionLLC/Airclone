...
| Dual-engine: librclone backend | [../plans/store-automation-plan.md](../plans/store-automation-plan.md) | `PLANNED` | In-process `LibRcloneClient` (dart:ffi) behind the `RcloneClient` seam — DMG ships both engines, MAS/iOS ship librclone-only; unblocks MAS (in-process I/O holds sandbox grants) and is a hard iOS prerequisite. |

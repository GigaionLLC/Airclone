...
| Reviewing or merging an outside pull request | [CONTRIBUTING](CONTRIBUTING.md) | [CLA](CLA.md). Never merge one, or copy code from one or from an issue, until its author's `Airclone CLA` status is green. Signing assigns the copyright to Gigaion, which must own every line to license it on non-AGPL terms (the App Store builds depend on it) |
